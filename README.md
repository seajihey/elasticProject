...
**문제 1. 연령대별 소비 패턴 분석의 한계**  
원본 데이터에는 사용자의 연령 정보가 포함되어 있었으나, Kibana 상에서 이를 연령대별로 구분하여 소비 패턴을 분석하지 못하는 문제가 발생했습니다. 그 결과 연령대 간 소비 성향 비교 및 주요 타깃 고객군 도출이 제한되었고, 분석 결과의 활용 범위가 축소되었습니다.

**문제 2. 프로젝트 초기 단계의 협업 및 커뮤니케이션 문제**  
프로젝트 초반에 명확한 분석 목표와 공통된 방향성에 대한 합의 없이 분석이 병렬적으로 진행되면서 작업 중복이 발생했습니다. 또한 팀원 간 데이터 해석 기준과 분석 관점이 달라, 중간 결과를 공유하는 과정에서 분석 방향성이 일관되지 않는 문제가 지속적으로 나타났습니다.
...
**문제 1 관련 원인**  
연령 정보가 단일 수치 컬럼으로만 존재했으며, 이를 연령대 단위의 그룹 컬럼으로 전처리하지 않은 상태에서 시각화를 시도한 것이 주요 원인이었습니다. 또, 프로젝트 초기 단계에서 각 데이터 컬럼의 의미와 활용 가능성에 대한 충분한 토의가 이루어지지 않아 분석에 적합한 파생 변수를 사전에 설계하지 못했습니다. 이로 인해 Kibana의 집계 및 시각화 기능을 충분히 활용하지 못했습니다.

**문제 2 관련 원인**  
프로젝트 시작 시 분석 목표, 역할 분담에 대한 사전 합의가 부족했습니다. 특히 데이터 탐색 과정을 거치지 않고 바로 분석에 들어가면서, 팀원마다 데이터에 대한 이해 수준과 문제 인식이 달랐습니다. 이로 인해 유사한 분석을 중복 수행하거나, 서로 다른 기준으로 데이터를 해석하는 등 협업 과정에서 비효율이 발생했습니다.
...
**문제 1 해결 방안**  
컬럼 정의서 및 전처리 체크리스트를 작성하여, 연령대와 같은 분석 핵심 변수를 사전에 명확히 정의했습니다. 본 프로젝트에서는 연령대 분석의 한계를 보완하기 위해 회원 등급을 대체 변수로 활용하여 소비 수준과 이용 패턴을 간접적으로 분석하는 방향으로 전략을 수정했습니다.

**문제 2 해결 방안**  
분석 주제와 최종 목표를 재정의한 후 팀원별 역할을 명확히 재분배하여 각자의 분석 범위를 구분했습니다. 데이터 해석 기준과 분석 방향을 지속적으로 조율했습니다. 개별 분석 결과 중심의 작업 방식에서 벗어나 결과 공유와 피드백을 중심으로 협업을 진행함으로써, 중복 작업을 최소화하고 팀 단위의 일관된 분석 결과를 도출할 수 있었습니다.
